...
public enum AlertStyle {
    ...
    case customImag(imageFile:UIImage)
}

open class SweetAlert: UIViewController {
    static let shared = SweetAlert()
    ...
    fileprivate func resizeAndRelayout() {
        ...
        if imageView != nil {
            imageView!.frame = CGRect(x: ((kContentWidth - kAnimatedViewHeight) / 2.0)-10, y: y, width: kAnimatedViewHeight+20, height: kAnimatedViewHeight)
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        ...
        case let .customImag(imageFile):
           // if let image = UIImage(named: imageFile) {
                self.imageView = UIImageView(image: imageFile)
    ...
    //MARK: - utilities
    func blankViewController(alert:SweetAlert) {
        alert.modalTransitionStyle = .crossDissolve
        blankViewController = UIViewController()
        blankViewController?.view.backgroundColor = UIColor.clear
        let window: UIWindow = UIApplication.shared.keyWindow!
        window.addSubview( (blankViewController?.view)!)
        window.bringSubview(toFront:  (blankViewController?.view)!)
        view.frame = window.bounds
        blankViewController?.present(alert, animated:true, completion: nil)
    }
    
    open func presentOkaySweetAlert(title:String, message: String, style:AlertStyle) {
        self.blankViewController(alert: SweetAlert().showAlert(title, subTitle: message, style: style))
    }
    
    open func presentSweetAlert(_ error: NSError) -> SweetAlert {
        let title = NSLocalizedString("Error", comment: "")
        let error_message = error.localizedDescription
        _ = self.presentOkaySweetAlert(title:title, message: error_message, style:.warning)
        return self
    }
